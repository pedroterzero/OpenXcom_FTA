	...
	_transformationRule(transformationRule), _base(base), _sourceSoldier(sourceSoldier), _filteredListOfSoldiers(filteredListOfSoldiers)
{
	...
	{
		_txtDescription = new Text(290, 26, 16, 27);
...
void SoldierTransformationState::initTransformationData()
{
	...
	bool noTransfer = transferTime <= 0;
	bool noRecovery = _transformationRule->getRecoveryTime() <= 0;
	bool noCost = _transformationRule->getCost() == 0;
	if (_game->getMod()->getIsFTAGame())
	{
	...
	{
		_txtDescription->setVisible(false);
		if (noCost)
		{
			_txtCost->setVisible(false);
			_txtTransferTime->setY(_txtTransferTime->getY() - 10);
			_txtRecoveryTime->setY(_txtRecoveryTime->getY() - 10);
		}
		if (noTransfer)
		{
			_txtTransferTime->setVisible(false);
	...
	else
	{
		...
		_lstRequiredItems->setVisible(false);
	}
