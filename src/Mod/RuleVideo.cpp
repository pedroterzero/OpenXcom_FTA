...
RuleVideo::RuleVideo(const std::string &id) : _id(id), _useUfoAudioSequence(false), _customCutscene(false), _winGame(false), _loseGame(false)
{
...
void RuleVideo::load(const YAML::Node &node)
{
	...
	_customCutscene = node["customCutscene"].as<bool>(false);
...
bool RuleVideo::customCutscene() const
{
	return _customCutscene;
}
